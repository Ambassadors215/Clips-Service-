...
3. **Cron:** `vercel.json` runs `/api/whatsapp-cron` **once per day** at 02:00 UTC (`0 2 * * *`). Vercel **Hobby** only allows a single run per day; an hourly schedule fails deployment. On Pro, you can set a more frequent `schedule` in `vercel.json` if needed, or use an **external** scheduler (e.g. GitHub Actions) to `GET/POST` this URL. On Vercel the request is authenticated via the `x-vercel-cron` header. Optionally set `CRON_WHATSAPP_SECRET` for manual GET/POST with `?secret=…` or `Authorization: Bearer`.
