...
export async function upsertMarketplaceListing(record) {
  ...
  return withRedis(async (client) => {
    ...
    if (Array.isArray(record.storeProducts) && record.storeProducts.length > 0) {
      priceListIn = record.storeProducts.map((p) => {
        const name = String(p?.name || p?.item || "").trim().slice(0, 120);
        const priceNum = Number(p?.price);
        const priceStr = Number.isFinite(priceNum)
          ? `£${priceNum.toFixed(2)}`
          : String(p?.price || "").trim().slice(0, 40);
        return { item: name, price: priceStr };
      });
    }
...
const devCustEmailVerifyOtpHash = new Map(); // email -> { hash, expMs }
...
export async function setCustomerEmailVerifyCooldown(email, sec = 45) {
  const e = normPushEmail(email);
  if (!e) return;
  if (!isKvRedisConfigured()) return;
  return withRedis(async (client) => {
    await client.setex(KEY_CUSTOMER_EMAIL_VERIFY_CD(e), sec, "1");
  });
}

export async function setCustomerEmailVerifyOtp(email, code, ttlSec = 900) {
  const e = normPushEmail(email);
  const hash = hashOtpCode(code);
  if (!isKvRedisConfigured()) {
    devCustEmailVerifyOtpHash.set(e, { hash, expMs: Date.now() + ttlSec * 1000 });
    return;
  }
  return withRedis(async (client) => {
    await client.setex(KEY_CUSTOMER_EMAIL_VERIFY_OTP(e), ttlSec, JSON.stringify({ hash }));
  });
}
