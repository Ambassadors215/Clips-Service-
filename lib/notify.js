...
export async function notifyProviderApplicationAdmin(record) {
  const html = `<p><strong>Provider application</strong></p>
<p>${esc(record.name)} · ${esc(record.email)}</p>
<pre style="font-family:sans-serif;white-space:pre-wrap">${esc(record.message)}</pre>`;
  await parallel(
    isEmailConfigured()
      ? sendEmail({
          to: adminInbox(),
