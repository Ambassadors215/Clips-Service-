...
import { sendTransactionalSms } from "./sms-send.js";
    ...
    isEmailConfigured()
      ? sendEmail({
          ...
          subject: isMarketplace
            ? `Your Clip Services order is confirmed — ${booking.ref}`
            : `Payment confirmed — ${booking.ref}`,
          html,
        })
      : Promise.resolve(),
    notifyPushUserEmail(booking.email, {
      title: isMarketplace ? "Order placed" : "Payment confirmed",
      body: isMarketplace ? `${booking.ref} — we'll notify you when the store updates` : `${booking.ref} — we’ll confirm on WhatsApp`,
      url: isMarketplace
        ? trackingMagicLink(booking) ||
          `${siteUrl()}/customer/orders/?email=${encodeURIComponent(booking.email.trim())}`
        : `/user/?email=${encodeURIComponent(booking.email.trim())}`,
    })
  );
}
...
<p>We’ve received your application to list your <strong>store</strong> on Clip Services.</p>
...
export async function notifyProviderApplicationAdmin(record, opts = {}) {
  ...
  const html = `<p><strong>${isStore ? "New store application" : "New provider application"}</strong></p>
...
/** After store marks order complete — ask customer for a quick star review (email + push). */
export async function notifyMarketplaceOrderCompletedReviewRequest(listing, booking, reviewUrl) {
  if (!booking?.email || !reviewUrl) return;
  const store = String(listing?.role || "the store").slice(0, 120);
  const html = `<p>Hi ${esc(booking.firstName)},</p>
<p>How was your order from <strong>${esc(store)}</strong>?</p>
<p><a href="${esc(reviewUrl)}">Leave a quick review (takes a few seconds)</a></p>
<p>Your feedback helps other shoppers discover great local businesses.</p>
<p>— Clip Services</p>`;
  await parallel(
    isEmailConfigured()
      ? sendEmail({
          to: booking.email,
          subject: `How was your order from ${store}? — Clip Services`,
          html,
        })
      : Promise.resolve(),
    notifyPushUserEmail(booking.email, {
      title: "How was your order?",
      body: `${store} — tap to leave a review`,
      url: reviewUrl,
    })
  );
}

/** Short SMS when Twilio env is configured (guest-friendly; respects UK numbers on booking.phone). */
export async function notifyMarketplaceOrderSmsStage(listing, booking, stage) {
  if (!booking?.marketplaceOrder || !booking?.phone || !booking?.ref) return;
  const store = String(listing?.role || "Store").trim().slice(0, 60);
  const ref = String(booking.ref);
  const isDel = String(booking.fulfillment || "") === "delivery";
  const map = {
    confirmed: `Clip Services:\nOrder #${ref} confirmed by ${store}.`,
    preparing: `Clip Services:\nOrder #${ref} is being prepared.`,
    ready: isDel ? "" : `Clip Services:\nOrder #${ref} is ready for collection.`,
    en_route: `Clip Services:\nOrder #${ref} is on the way.`,
    completed: `Clip Services:\nOrder #${ref} complete. Thanks for shopping with ${store}.`,
  };
  const body = map[stage];
  if (!body) return;
  await sendTransactionalSms(booking.phone, body);
}
