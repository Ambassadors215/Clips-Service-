...
/**
 * Display-only Title Case for store names that owners typed in lower case
 * during onboarding (e.g. "sonia africa" → "Sonia Africa"). Leaves existing
 * uppercase letters alone so "MK Foods" or "ABC Halal" stay intact. The raw
 * `listing.role` in storage is untouched.
 */
function displayStoreName(raw) {
  const s = String(raw || "").trim();
  if (!s) return "";
  if (/[A-Z]/.test(s)) return s;
  return s.replace(/\b([a-z])([a-z]*)/g, (_m, a, rest) => a.toUpperCase() + rest);
}
...
export function buildStorePageHtml(listing, opts) {
  ...
  const name = displayStoreName(listing.role) || "Independent store";
  ...
  const emptyShopHtml = `<div class="shop-empty" role="status">
  <div class="shop-empty-emoji" aria-hidden="true">🛒</div>
  <h3>Catalogue arriving soon</h3>
  <p>${esc(name)} is still adding products to Clip Services.${wa ? " Tap <strong>Message store</strong> above to ask what's in this week — they'll reply on WhatsApp." : " Check back shortly, or browse other Manchester independents while you wait."}</p>
  <div class="shop-empty-actions">
    ${waStoreUrl ? `<a class="btn-sec" href="${esc(waStoreUrl)}" target="_blank" rel="noopener">💬 Message ${esc(name)}</a>` : ""}
    <a class="btn-sec" href="/stores">Browse all Manchester stores →</a>
  </div>
</div>`;
  const mainGrid = mainProducts.length || popularIdx.length
    ? `<div class="pgrid main" id="catalogue">${mainProducts.map((p) => cardHtml(p, false)).join("")}</div>`
    : emptyShopHtml;
...
.shop-empty{background:var(--card);border:1px dashed rgba(139,58,58,.35);border-radius:18px;padding:32px 22px;text-align:center;margin:8px 0 24px}
.shop-empty-emoji{font-size:2.4rem;margin-bottom:10px;line-height:1}
.shop-empty h3{font-family:"Playfair Display",Georgia,serif;color:var(--terracotta);font-size:1.2rem;margin:0 0 8px}
.shop-empty p{margin:0 auto 16px;max-width:520px;color:#5c4033;font-size:.95rem;line-height:1.5}
.shop-empty-actions{display:flex;flex-wrap:wrap;justify-content:center;gap:10px}
  ...
  <div class="hero-inner">
    <div class="hero-card">
      ...
      <div class="cta-row">
        ...
        ${waStoreUrl ? `<a class="btn-sec" href="${esc(waStoreUrl)}" target="_blank" rel="noopener" aria-label="Message ${esc(name)} on WhatsApp">💬 Message store</a>` : `<a class="btn-sec" href="mailto:clipservices26@gmail.com?subject=Question%20about%20${encodeURIComponent(name)}" aria-label="Email Clip Services about this store">💬 Message store</a>`}
...
<div class="layout">
  ...
  ${bio.length > 220 ? `<div id="desc-full" class="hidden" style="display:none">${descFull}</div><button type="button" class="read-tog" id="read-more">Read more</button>` : ""}
</div>`
        : ""
    }
  ...
  <form method="dialog" id="chk-form" novalidate>
    ...
    <div class="dlg-b">
      ...
      <div id="step-3" class="hidden" style="display:none">
        ...
        <label><input type="checkbox" id="consent" required /> I agree to the <a href="/terms" target="_blank" rel="noopener">terms</a> and processing of my data for this order.</label>
      </div>
...
(function(){
  ...
  function goStep(s){
    ...
    for(var i=1;i<=4;i++){
      ...
      if(p){ p.classList.toggle("hidden", i!==s); p.style.display=i===s?"block":"none"; }
      var si=document.querySelector("#steps i[data-s=\""+i+"\"]");
      if(si) si.classList.toggle("on", i===s);
    }
    var bBack=document.getElementById("chk-back");
    var bNext=document.getElementById("chk-next");
    var bPay=document.getElementById("chk-pay");
    if(bBack){ bBack.classList.toggle("hidden", !(s>1)); bBack.style.display=s>1?"inline-block":"none"; }
    if(bNext){ bNext.classList.toggle("hidden", !(s<4)); bNext.style.display=s<4?"inline-block":"none"; }
    if(bPay){ bPay.classList.toggle("hidden", s!==4); bPay.style.display=s===4?"inline-block":"none"; }
    if(s===4){
