...
@media(min-width:960px){.layout{grid-template-columns:1fr;align-items:start;gap:0;padding-bottom:48px}}
.store-nav{position:sticky;top:0;z-index:50;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 16px;background:rgba(253,251,248,.96);backdrop-filter:blur(8px);border-bottom:1px solid rgba(139,58,58,.15);max-width:1180px;margin:0 auto}
...
.qty.hidden,.added.hidden{display:none!important}
html.clip-offline .btn-add,html.clip-offline #proceed-checkout-drawer,html.clip-offline #open-basket-mob,html.clip-offline #nav-basket{opacity:.5;pointer-events:none}
</style>
</head>
<body>
<nav class="store-nav">
  <div><a href="/">← Marketplace</a> · <a href="/stores">All stores</a></div>
  <button type="button" class="nav-cart" id="nav-basket" aria-label="Open basket">🛒 <span class="nb" id="nav-badge">0</span></button>
</nav>
...
(function(){
  ...
  function goStep(s){
    ...
    if(s===4){
      ...
      if(pa) pa.textContent="Your payment is secure and encrypted by Stripe. Pay £"+total().toFixed(2)+" — Apple Pay and Google Pay available where supported.";
    }
  }
  ...
  document.getElementById("chk-form").onsubmit=function(ev){
    ...
    body.orderSource=(window.matchMedia && window.matchMedia("(display-mode: standalone)").matches) || window.navigator.standalone ? "pwa" : "web";
