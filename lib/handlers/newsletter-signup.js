...
export default async function handler(req, res) {
  ...
  if (!isValidEmail(email)) {
    ...
    res.statusCode = 303;
    res.setHeader("Location", buildRedirect(base, nextPath || "/", "invalid"));
    res.end();
    return;
  }
  ...
  const normalisedSource = (source || "clip-services-footer").slice(0, 80);
  ...
  try {
    ...
    res.end('<!DOCTYPE html><html lang="en-GB"><head><meta charset="UTF-8"><title>Signup unavailable</title></head><body><p>Signup is temporarily unavailable. Please try again shortly or email <a href="mailto:clipservices26@gmail.com">clipservices26@gmail.com</a>.</p><p><a href="/">← Home</a></p></body></html>');
    return;
  }
  ...
  res.statusCode = 303;
  res.setHeader("Location", buildRedirect(base, nextPath || "/", "ok"));
  res.end();
}
