...
/** Stable id for the signup / primary saved delivery row (upsert on complete-account-setup). */
const SIGNUP_PRIMARY_ADDRESS_ID = "clip_primary_home";
...
async function loadOrdersForEmail(email) {
  ...
  for (const b of marketplace) {
    ...
    orders.push({
      ...
      orderStageHistory: Array.isArray(b.orderStageHistory) ? b.orderStageHistory : [],
...
export default async function handler(req, res) {
  const email = await getCustomerSessionEmailFromReq(req);
  ...
  try {
    ...
    if (action === "put-marketplace-cart") {
      const listingId = clampStr(payload?.listingId, 80);
      const cart = payload?.cart;
      if (!listingId || !cart || typeof cart !== "object") {
        return endJson(res, 400, { ok: false, error: "listingId and cart object required" });
      }
      const cleaned = {};
      for (const [k, v] of Object.entries(cart)) {
        const key = String(k).trim();
        if (!/^\d+$/.test(key)) continue;
        const q = Math.min(99, Math.max(0, Math.floor(Number(v) || 0)));
        if (q > 0) cleaned[key] = q;
        if (Object.keys(cleaned).length >= 50) break;
      }
      const profile = await ensureCustomerProfile(email);
      const prev =
        profile && typeof profile.marketplaceCarts === "object" && profile.marketplaceCarts
          ? { ...profile.marketplaceCarts }
          : {};
      prev[listingId] = cleaned;
      const updated = await patchCustomerProfile(email, { marketplaceCarts: prev });
      return endJson(res, 200, { ok: true, marketplaceCarts: updated?.marketplaceCarts || {} });
    }
    ...
    if (action === "complete-account-setup") {
      ...
      const addressLine1 = clampStr(payload.addressLine1, 120);
      const addressLine2 = clampStr(payload.addressLine2, 120);
      const city = clampStr(payload.city, 60);
      ...
      if (addressLine1.length < 5) {
        return endJson(res, 400, { ok: false, error: "Enter your full street address (line 1)." });
      }
      if (!city || city.length < 2) {
        return endJson(res, 400, { ok: false, error: "Enter your town or city." });
      }
      const profile0 = await ensureCustomerProfile(email);
      const existing = Array.isArray(profile0?.addresses) ? profile0.addresses : [];
      const rest = existing.filter((a) => a && a.id !== SIGNUP_PRIMARY_ADDRESS_ID);
      const signupHome = {
        id: SIGNUP_PRIMARY_ADDRESS_ID,
        label: "Home",
        line1: addressLine1,
        line2: addressLine2,
        city,
      ...
      const nextAddresses = [signupHome, ...rest].slice(0, 8);
