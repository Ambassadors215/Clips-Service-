...
export default async function handler(req, res) {
  ...
  const matched = listings.filter(
    (L) => listingIsManchesterCustomerBrowse(L) && listingMatchesCategory(L, copy.keywords)
  );
  const listHtml = matched.length
    ? `<div style="display:grid;gap:14px;margin:24px 0">${matched
        .map((L) => {
          const sid = encodeURIComponent(publicStoreSlug(L));
          const n = esc(L.role || "Store");
          return `<article style="padding:14px;background:#fff;border-radius:12px;border:1px solid #E8DFD4"><a href="/stores/${sid}" style="font-weight:700;color:#8B3A3A">${n}</a><br><span style="font-size:14px;color:#5C4033">${esc((L.bio || "").slice(0, 160))}${(L.bio && L.bio.length > 160 ? "…" : "")}</span></article>`;
        })
        .join("")}</div>`
    : `<p style="padding:18px;background:#faf6f2;border-radius:12px;border:1px dashed #c4b8a8"><strong>More Manchester stores are onboarding.</strong> Browse <a href="/stores">live stores</a> or <a href="/store-owner">list your shop</a>.</p>`;

  const areaLinks = MANCHESTER_AREAS.map(
  ...
  const catNavLinks = CATEGORY_SLUGS.filter((s) => s !== slug)
    .map((s) => `<a href="/categories/${encodeURIComponent(s)}">${esc(CATEGORY_NAV_LABELS[s] || s)}</a>`)
    .join(" · ");
...
<p>${esc(copy.intro)}</p>
<h2 style="font-size:1.1rem;margin-top:28px">Stores selling in this category</h2>
${listHtml}
<h2 style="font-size:1.1rem;margin-top:32px">Explore other categories</h2>
<p style="font-size:14px;line-height:2">${catNavLinks}</p>
<h2 style="font-size:1.1rem;margin-top:28px">FAQ</h2>
...
<details style="margin-bottom:10px;background:#fff;padding:14px;border-radius:10px;border:1px solid #E8DFD4"><summary>Do you deliver nationwide?</summary><p>Clip Services is launching in Greater Manchester first. Each live store sets collection and local delivery — availability is shown on the store page.</p></details>
<h2 style="font-size:1.1rem;margin-top:28px">Browse by Manchester area</h2>
<p style="font-size:14px;line-height:2">${areaLinks}</p>
<p style="margin-top:24px"><a href="/stores">Browse Manchester stores</a> · <a href="/store-owner" style="font-weight:700;color:#8B3A3A">List your store</a></p>
</div>
