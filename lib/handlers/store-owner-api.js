...
import { notifyMarketplaceOrderCompletedReviewRequest } from "../notify.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "set-order-stage") {
      ...
      const orderTrackUrl = `${siteUrl()}/customer/orders/?email=${encodeURIComponent(customerEmail)}`;
      ...
      if (customerEmail && stage === "ready") {
        ...
        void notifyPushUserEmail(customerEmail, {
          title: "Order update",
          body: isDel ? `Your order from ${sn} is on the way 🚗` : `Your order from ${sn} is ready! 🎉`,
          url: orderTrackUrl,
        }).catch(() => {});
      }
