...
import {
  ...
  getBookingByRef,
  getRecentSiteVisits,
  getStoreAnalyticsCounters,
} from "../kv-store.js";
import { publicStoreSlug } from "../seo-html.js";
import { siteUrl } from "../site-url.js";
import { notifyMarketplaceOrderCompletedReviewRequest, notifyMarketplaceOrderSmsStage } from "../notify.js";
import { notifyPushUserEmail } from "../push.js";
import {
  waNotifyCustomerOrderStage,
  waScheduleOrderCompletedJobs,
} from "../whatsapp-notify.js";
import { trackingMagicLink } from "../order-utils.js";
...
function deriveStage(b) {
  if (b.storeStage && ["new", "confirmed", "preparing", "en_route", "ready", "completed"].includes(b.storeStage)) {
    return b.storeStage;
  }
  const st = String(b.status || "");
  if (st === "awaiting_payment" || st === "new") return "new";
  if (st === "paid") return "new";
...
async function buildSnapshot(email, listingIdIn) {
  ...
  let storeFunnel = { view: 0, add_basket: 0, checkout_start: 0, order_complete: 0, productAdds: {} };
  try {
    const sf = await getStoreAnalyticsCounters(String(listing.id));
    if (sf) storeFunnel = sf;
  } catch {
    /* ignore */
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "set-order-stage") {
      ...
      if (!ref || !["new", "confirmed", "preparing", "en_route", "ready", "completed"].includes(stage)) {
      ...
      const prevHist = Array.isArray(b.orderStageHistory) ? [...b.orderStageHistory] : [];
      prevHist.push({ status: stage, at: new Date().toISOString(), note: "" });
      await patchBooking(ref, { storeStage: stage, orderStageHistory: prevHist });
      const customerEmail = String(b.email || "")
        .trim()
        .toLowerCase();
      const orderTrackUrl =
        trackingMagicLink(b) || `${siteUrl()}/customer/orders/?email=${encodeURIComponent(customerEmail)}`;
      let whatsappUrl = "";
      let reviewWhatsappUrl = "";
      if (stage === "ready" && b.phone) {
        const storeName0 = listing.role || "our store";
        const cust = `${b.firstName || ""}`.trim() || "there";
        const isDel0 = String(b.fulfillment) === "delivery";
        const msg = isDel0
          ? `Hi ${cust} 👋 Your order from ${storeName0} is ready to go out for delivery soon. We’ll let you know when it’s on the way.`
          : `Hi ${cust} 👋 Your order from ${storeName0} is ready for collection. Thank you for supporting local businesses 🙌🏾`;
        const digits = String(b.phone).replace(/\D/g, "");
        if (digits.length >= 10) {
          whatsappUrl = `https://wa.me/${digits}?text=${encodeURIComponent(msg)}`;
        }
      }
      if (stage === "completed" && b.phone) {
        const base = siteUrl();
        const reviewPath = `/customer/review/?ref=${encodeURIComponent(ref)}&email=${encodeURIComponent(customerEmail)}&src=wa`;
        const reviewUrl = `${base}${reviewPath}`;
        const name = listing.role || "our store";
        const rmsg = `How was your order from ${name}? Tap to leave a quick review: ${reviewUrl}`;
        let d = String(b.phone).replace(/\D/g, "");
        if (d.length >= 10) {
          if (d.startsWith("0")) d = "44" + d.slice(1);
          else if (!d.startsWith("44")) d = "44" + d;
          reviewWhatsappUrl = `https://wa.me/${d}?text=${encodeURIComponent(rmsg)}`;
        }
        void waScheduleOrderCompletedJobs(listing, b).catch((e) => console.error("WA_SCHED", e));
        void notifyMarketplaceOrderCompletedReviewRequest(listing, b, reviewUrl).catch((e) => console.error("NOTIFY_REVIEW_REQ", e));
      }
      void waNotifyCustomerOrderStage(listing, b, stage).catch((e) => console.error("WA_STAGE", e));
      void notifyMarketplaceOrderSmsStage(listing, b, stage).catch((e) => console.error("SMS_STAGE", e));
      if (customerEmail && stage === "confirmed") {
        const sn = String(listing.role || "The store").slice(0, 100);
        void notifyPushUserEmail(customerEmail, {
          title: "Order confirmed",
          body: `${sn} accepted your order ${b.ref}`,
          url: orderTrackUrl,
        }).catch(() => {});
      }
      if (customerEmail && stage === "preparing") {
        const sn = String(listing.role || "The store").slice(0, 100);
        void notifyPushUserEmail(customerEmail, {
          title: "Order being prepared",
          body: `Your order from ${sn} is being prepared 👨🏾‍🍳`,
          url: orderTrackUrl,
        }).catch(() => {});
      }
      if (customerEmail && stage === "ready") {
        const sn = String(listing.role || "The store").slice(0, 100);
        const isDel = String(b.fulfillment) === "delivery";
        void notifyPushUserEmail(customerEmail, {
          title: "Order update",
          body: isDel ? `Your order from ${sn} is packed for delivery` : `Your order from ${sn} is ready! 🎉`,
          url: orderTrackUrl,
        }).catch(() => {});
      }
      if (customerEmail && stage === "en_route") {
        const sn = String(listing.role || "The store").slice(0, 100);
        void notifyPushUserEmail(customerEmail, {
          title: "On the way",
          body: `Your delivery from ${sn} is heading to you`,
          url: orderTrackUrl,
        }).catch(() => {});
      }
      if (customerEmail && stage === "completed") {
        const sn = String(listing.role || "The store").slice(0, 100);
        void notifyPushUserEmail(customerEmail, {
          title: "Order complete",
          body: `Thanks for ordering from ${sn}`,
          url: orderTrackUrl,
        }).catch(() => {});
      }
      return endJson(res, 200, { ok: true, whatsappUrl, reviewWhatsappUrl });
    }
