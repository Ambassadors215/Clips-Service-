...
function deriveStage(b) {
  if (b.storeStage && ["new", "confirmed", "preparing", "ready", "completed"].includes(b.storeStage)) {
    return b.storeStage;
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "set-order-stage") {
      ...
      if (!ref || !["new", "confirmed", "preparing", "ready", "completed"].includes(stage)) {
      ...
      if (stage === "completed" && b.phone) {
        const base = siteUrl();
        const reviewPath = `/customer/review/?ref=${encodeURIComponent(ref)}&email=${encodeURIComponent(customerEmail)}`;
