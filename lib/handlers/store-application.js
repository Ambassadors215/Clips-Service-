...
import { waNotifyStoreOwnerApplicationWelcome } from "../whatsapp-notify.js";
...
export default async function handler(req, res) {
  ...
  if (flow === "stall") {
    return endJson(res, 410, {
      ok: false,
      error:
        "Market stall applications are no longer available. Apply to list your store instead — same onboarding flow.",
      redirect: "/store-owner/apply",
    });
  }
...
async function handleStoreFlow(req, res, payload) {
  ...
  void waNotifyStoreOwnerApplicationWelcome({
    id: listingId,
    role: storeName,
    whatsappPhone: whatsapp,
    ownerName,
  ...
  void scheduleWaJob(Date.now() + 48 * 60 * 60 * 1000, { kind: "wa_onboard_48h", listingId: String(listingId) });
