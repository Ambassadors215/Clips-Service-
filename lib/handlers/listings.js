...
export default async function handler(req, res) {
  ...
  try {
    const rows = (await getMarketplaceListings())
      ...
      .filter(listingIsManchesterCustomerBrowse);
    const items = rows.map(({ email: _e, ...pub }) => {
      ...
      return {
        ...
        deliveryEnabled: pub.deliveryEnabled !== false,
        collectionEnabled: pub.collectionEnabled !== false,
        storeOpen: pub.storeOpen !== false,
        postcode: typeof pub.postcode === "string" ? pub.postcode.trim().slice(0, 20) : "",
        productPreview,
      };
    });
