...
import { allocateUniqueMarketplaceOrderRef, newTrackingToken } from "../order-utils.js";
...
export default async function handler(req, res) {
  ...
  /* ── Marketplace store basket (listingId + cartLines) ── */
  const listingIdRaw = String(payload?.listingId ?? "")
    .replace(/[\u200B-\u200D\uFEFF\u00A0]/g, "")
    .trim()
    .slice(0, 64);
  const cartLinesIn = payload?.cartLines;
  if (listingIdRaw && Array.isArray(cartLinesIn) && cartLinesIn.length) {
    const listing = await getMarketplaceListingById(listingIdRaw);
    if (!listing) {
      return endJson(res, 404, { ok: false, error: "Store not found" });
    }
    const priceList = catalogPriceRowsForListing(listing);
    if (!priceList.length) {
      return endJson(res, 400, { ok: false, error: "This store has no priced products yet" });
    }
    ...
    const fulfillment = safeText(payload?.fulfillment, 20).toLowerCase() === "delivery" ? "delivery" : "collection";
    ...
    if (fulfillment === "delivery" && listing.deliveryEnabled === false) {
      return endJson(res, 400, { ok: false, error: "Delivery is not available from this store" });
    }
    if (fulfillment === "collection" && listing.collectionEnabled === false) {
      return endJson(res, 400, { ok: false, error: "Collection is not available from this store" });
    }

    let deliveryAddress = null;
    if (fulfillment === "delivery") {
      const addr = payload?.deliveryAddress && typeof payload.deliveryAddress === "object" ? payload.deliveryAddress : {};
      const line1 = safeText(addr.line1, 200);
      const cityA = safeText(addr.city, 80);
      const postcode = safeText(addr.postcode, 12);
      if (!line1 || !cityA || !postcode) {
        return endJson(res, 400, { ok: false, error: "Enter your full delivery address" });
      }
      deliveryAddress = { line1, city: cityA, postcode };
    }

    const resolved = [];
    for (const line of cartLinesIn) {
      if (!line || typeof line !== "object") continue;
      const idx = Number(line.idx);
      const qty = Number(line.qty);
      if (!Number.isInteger(idx) || idx < 0 || idx >= priceList.length) continue;
      if (!Number.isInteger(qty) || qty < 1 || qty > 99) continue;
      const row = priceList[idx];
      if (!row?.item || !row?.price) continue;
      const unit = parsePriceGBP(row.price);
      if (unit <= 0) continue;
      resolved.push({ idx, qty, row, unit });
    }
    if (!resolved.length) {
      return endJson(res, 400, { ok: false, error: "Cart is empty or invalid" });
    }

    let subtotalGBP = 0;
    for (const { qty, unit } of resolved) {
      subtotalGBP += qty * unit;
    }
    subtotalGBP = Math.round(subtotalGBP * 100) / 100;
    const minListing = Number(listing.minimumOrderGbp);
    if (Number.isFinite(minListing) && minListing > 0 && subtotalGBP < minListing) {
      return endJson(res, 400, {
        ok: false,
        error: `Minimum order for this store is £${minListing.toFixed(2)}`,
      });
    }
    if (subtotalGBP < 5 || subtotalGBP > 5000) {
      return endJson(res, 400, { ok: false, error: "Order total must be between £5 and £5,000" });
    }

    const deliveryFeeGBP =
      fulfillment === "delivery"
        ? Math.round((Number(listing.deliveryChargeGbp) || 0) * 100) / 100
        : 0;

    const storeName = safeText(listing.role, 120) || "Independent store";
    const linesDesc = resolved.map(({ qty, row }) => `${qty}× ${row.item}`).join("; ").slice(0, 2800);
    const service = `Store order: ${storeName}`.slice(0, 120);
    const fee = Math.round(subtotalGBP * 0.15 * 100) / 100;
    const total = Math.round((subtotalGBP + deliveryFeeGBP + fee) * 100) / 100;
    const unitAmountPence = Math.round(total * 100);
    if (unitAmountPence < 30) return endJson(res, 400, { ok: false, error: "Amount too small" });

    const ref = await allocateUniqueMarketplaceOrderRef(getBookingByRef);
    const trackingToken = newTrackingToken();
    const createdAt = new Date().toISOString();
    const addrLine =
      deliveryAddress != null
        ? `\nDeliver to: ${deliveryAddress.line1}, ${deliveryAddress.city} ${deliveryAddress.postcode}`
        : "";
    const notes = `${linesDesc}\nFulfillment: ${fulfillment}${addrLine}`.slice(0, 3000);
    const record = {
      ref,
      createdAt,
      ...
      totalGBP: total,
      dur: "",
      date: fulfillment === "delivery" ? "Delivery" : "Collection",
      time: "—",
      firstName,
      lastName,
      phone,
      ...
      orderSource: (() => {
        const o = String(payload?.orderSource || "").toLowerCase();
        if (o === "pwa") return "pwa";
        if (o === "wa" || o === "whatsapp") return "wa";
        return "web";
      })(),
      cartSnapshot: resolved.map(({ idx, qty, row }) => ({ idx, qty, item: row.item, price: row.price })),
      fulfillment,
      deliveryAddress,
      storeStage: "new",
      trackingToken,
      orderStageHistory: [{ status: "new", at: createdAt, note: "Order started — complete payment to confirm" }],
    };

    const providerId = String(listing.id);
    const connectedAccountId = await resolveConnectAccount(providerId);
    const providerSharePence = Math.round((subtotalGBP + deliveryFeeGBP) * 100);

    const metaBookingRef = String(ref).replace(/\0/g, "").slice(0, 500);
    const paymentIntentData = {
      metadata: { bookingRef: metaBookingRef, listingId: String(listing.id).slice(0, 100) },
    };
    ...
    try {
      ...
      const descParts = [
        linesDesc.slice(0, 320),
        deliveryFeeGBP > 0 ? `Delivery £${deliveryFeeGBP.toFixed(2)}.` : "",
        `Total £${total.toFixed(2)} incl. service charge.`,
      ]
        .filter(Boolean)
        .join(" ");
      ...
      const session = await createCheckoutSessionWithConnectFallback(stripe, {
        ...
        line_items: [
          {
            price_data: {
              currency: "gbp",
              product_data: {
                name: `Clip Services — ${storeName.slice(0, 60)}`,
                description: descParts.slice(0, 500),
              },
        ...
        success_url: `${origin}/customer/order-confirmed?paid=1&ref=${encodeURIComponent(ref)}&email=${encodeURIComponent(email)}&track=${encodeURIComponent(trackingToken)}`,
        cancel_url: `${origin}/stores/${encodeURIComponent(publicStoreSlug(listing))}?cancel=1`,
      ...
      const storeEmail = String(listing.email || "")
        .trim()
        .toLowerCase();
      void Promise.allSettled([
        notifyCheckoutStartedCustomer(record, session.url),
        notifyCheckoutStartedAdmin(record),
        storeEmail && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(storeEmail)
          ? notifyCheckoutStartedProvider(storeEmail, record)
          : Promise.resolve(),
      ]);

      return endJson(res, 200, { ok: true, url: session.url, ref });
    } catch (e) {
      console.error("STRIPE_CHECKOUT_MARKETPLACE_ERROR", e);
      const pub = publicCheckoutError(e, "MARKETPLACE_CHECKOUT");
  ...
  try {
    ...
    const session = await createCheckoutSessionWithConnectFallback(stripe, {
      ...
      success_url: `${origin}/?booking=paid&ref=${encodeURIComponent(ref)}`,
      cancel_url: `${origin}/?booking=cancel&ref=${encodeURIComponent(ref)}`,
      customer_email: email,
      client_reference_id: String(ref).slice(0, 200),
      metadata: { bookingRef: legacyMetaRef },
      payment_intent_data: paymentIntentData,
