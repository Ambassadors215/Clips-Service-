import { getBookingByRef, getMarketplaceListingById, patchBooking, upsertMarketplaceListing, incrementStoreAnalytics, incrementPlatformFunnel, incrementWhatsAppMetric } from "../kv-store.js";
import { notifyPushProviderEmail } from "../push.js";
import { siteUrl } from "../site-url.js";
import { waNotifyStoreNewReview } from "../whatsapp-notify.js";
...
export default async function handler(req, res) {
  ...
  const text = String(body?.text || "")
    .trim()
    .slice(0, 2000);
  const fromWa = String(body?.source || "").toLowerCase() === "wa";
  ...
  try {
    ...
    if (fromWa) {
      void incrementWhatsAppMetric("review_submitted_from_wa", 1);
    }

    void waNotifyStoreNewReview(listing, {
      rating,
      text,
      customerFirstName: String(b.firstName || "Customer").trim().slice(0, 80) || "Customer",
    }).catch(() => {});
