...
import { listingIsManchesterCustomerBrowse } from "../manchester-scope.js";
...
function pickSimilarStores(listing, all) {
  ...
  const candidates = all.filter(
    (x) =>
      isListingPubliclyVisible(x) &&
      listingIsManchesterCustomerBrowse(x) &&
      String(x.id) !== id
  );
...
export default async function handler(req, res) {
  ...
  if (!isListingPubliclyVisible(listing)) {
    ...
    res.setHeader("content-type", "text/html; charset=utf-8");
    res.end(
      `<!DOCTYPE html><html lang="en-GB"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Store not found</title></head><body style="font-family:system-ui,sans-serif;padding:24px;background:#F5F0E8;color:#2C1810"><p>This store listing is no longer available.</p><p><a href="/stores">Browse stores</a></p></body></html>`
    );
    return;
  }

  if (!listingIsManchesterCustomerBrowse(listing)) {
    res.statusCode = 404;
    res.setHeader("content-type", "text/html; charset=utf-8");
    res.end(
      `<!DOCTYPE html><html lang="en-GB"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Store not found</title></head><body style="font-family:system-ui,sans-serif;padding:24px;background:#F5F0E8;color:#2C1810"><p>This store isn’t listed on the Greater Manchester marketplace yet.</p><p><a href="/stores">Browse Manchester stores</a></p></body></html>`
    );
    return;
  }
  ...
  const mapsApiKey = String(process.env.GOOGLE_MAPS_API_KEY || process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || "").trim();
  const html = buildStorePageHtml(listing, { similarStores, base, citySlug, mapsApiKey });
