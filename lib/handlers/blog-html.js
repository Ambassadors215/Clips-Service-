...
function cardHtml(p, base) {
  ...
  return `<article class="blog-card"><a class="blog-card-thumb" href="/blog/${esc(p.slug)}" tabindex="-1"><img loading="lazy" decoding="async" src="${src}" width="640" height="360" alt="${alt}"></a><div class="blog-card-body"><span class="blog-badge">${esc(
    lab(p.category)
  )}</span><h3><a href="/blog/${esc(p.slug)}">${esc(p.title)}</a></h3><p class="blog-excerpt">${esc(p.excerpt || p.metaDescription)}</p><div class="blog-card-meta">${Number(p.readMinutes) || "?"} min read · ${esc(
    p.datePublished
...
function heroFeatured(post, base) {
  ...
  return `<section class="blog-hero"><span class="blog-hero-kicker">Featured</span><div class="blog-hero-card"><div class="blog-card-thumb"><a href="/blog/${esc(post.slug)}"><img src="${src}" alt="${alt}" loading="lazy" /></a></div><div class="blog-hero-body"><h2><a href="/blog/${esc(post.slug)}">${esc(post.title)}</a></h2><p>${esc(ex)}${ex.length >= 260 ? "…" : ""}</p><p class="blog-hero-meta">${Number(post.readMinutes) || "?"} min · ${esc(post.author)} · ${esc(
    post.datePublished
  )}</p><a class="blog-cta" href="/blog/${esc(post.slug)}">Read article</a></div></div></section>`;
}
...
${indexLd}</head><body><div id="global-search-mount"></div><div class="blog-max"><nav class="blog-nav"><a href="/" class="blog-logo">Clip Services</a><a href="/blog">Blog home</a><a href="/stores">Stores</a><a href="/search">Search</a><a href="/blog/rss.xml">RSS</a></nav>
...
<meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${esc(post.title)}"><meta name="twitter:image" content="${esc(imgAbs)}"><meta name="theme-color" content="#8B3A3A">
...
${jsonLdScript(ld)}</head><body><div id="global-search-mount"></div><div class="blog-max"><nav class="blog-nav"><a href="/" class="blog-logo">Clip Services</a><a href="/blog">Blog home</a><a href="/stores">Stores</a></nav>
...
export default async function handler(req, res) {
  ...
  sendLanding(res, {
    ...
    desc: "Stories, guides & recipes linking African, Caribbean & Asian indie stores on Clip Services Ltd.",
