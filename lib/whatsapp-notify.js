...
const ORDERS = (booking) =>
  booking?.trackingToken && booking?.ref
    ? trackingMagicLink(booking) || fallbackOrdersEmail(booking)
    : fallbackOrdersEmail(booking);

function fallbackOrdersEmail(booking) {
  return `${siteUrl()}/customer/orders/?email=${encodeURIComponent(String(booking?.email || "").trim())}&src=wa`;
}
...
export async function waNotifyCustomerOrderPlaced(listing, booking) {
  ...
  const text = `Hi ${name} 👋
Your order #${booking.ref} from ${sn} has been received successfully.
Track live: ${ORDERS(booking)}`;
...
export async function waNotifyCustomerOrderStage(listing, booking, stage) {
  ...
  if (!to) return;
  const sn = storeName(listing);
  const nm = firstName(booking);
  const isDel = String(booking.fulfillment) === "delivery";

  if (stage === "confirmed") {
    const text = `✅ Your order #${booking.ref} has been confirmed by ${sn}.
Track: ${ORDERS(booking)}`;
    await sendWhatsAppText(to, text, { type: "order_confirmed_customer" });
    return;
  }
  if (stage === "preparing") {
    const text = `${nm}, your store is preparing your order #${booking.ref}. We’ll ping you when it’s ready 🍽️
Track: ${ORDERS(booking)}`;
    await sendWhatsAppText(to, text, { type: "order_preparing_customer" });
    return;
  }
  if (stage === "ready" && isDel) {
    const text = `🎉 Your order #${booking.ref} is packed and heading out soon.
Store: ${storeAddressLine(listing)}
Track: ${ORDERS(booking)}`;
    await sendWhatsAppText(to, text, { type: "order_ready_delivery" });
    return;
  }
  if (stage === "ready" && !isDel) {
    const text = `🎉 Your order #${booking.ref} is ready for collection.
Store: ${storeAddressLine(listing)}
Track: ${ORDERS(booking)}`;
  ...
  if (stage === "en_route" && isDel) {
    const text = `🚗 Your order #${booking.ref} is on the way.
Track: ${ORDERS(booking)}`;
...
Leave a quick review when you have a minute:
${rUrl}`;
